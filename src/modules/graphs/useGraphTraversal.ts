...
import type { GraphAlgorithm, GraphFrame } from './engine'

const PLAY_INTERVAL = 800

export default function useGraphTraversal(algorithm: GraphAlgorithm = 'dfs') {
  ...
  const setup = useCallback(() => {
    ...
    setIsPlaying(false)
    setFrames(computeGraphFrames(algorithm))
    setFrameIdx(0)
  }, [algorithm])
