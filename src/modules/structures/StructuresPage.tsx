...
function ArrayViz() {
  ...
  function fail(text: string) { setErr(true); setMsg(text); setTimeout(() => setErr(false), 2000) }
    ...
    <div className={styles.vizWrap}>
      ...
      <div className={styles.stackCol}>
        ...
        {[...stack].reverse().map((v, ri) => {
          const isTop = ri === 0
          return (
            <div key={stack.length - 1 - ri} className={`${styles.stackItem} ${isTop && topFlash ? styles.cellLit : styles.cellIdle}`}>
              {v}
  ...
  return (
    <div className={styles.vizWrap}>
      ...
      <div className={styles.queueRow}>
        ...
        {queue.map((v, i) => (
          <div key={i} className={styles.cellWrap}>
            <div className={`${styles.cell} ${i === 0 && flashFront ? styles.cellLit : styles.cellIdle}`}>
              {v}
            </div>
      ...
      <div className={styles.linkedRow}>
        ...
        {list.map((v, i) => (
          <div key={i} className={styles.llNode}>
            <div className={`${styles.llCell} ${highlight === i ? styles.cellLit : styles.cellIdle}`}>
              {v}
            </div>
...
function layoutBST(root: BSTNode | null, x: number, y: number, gap: number, result: { nodes: NodePos[]; edges: EdgePos[] }, parentX?: number, parentY?: number, id = '0') {
  if (!root) return
...
function BSTViz() {
  ...
  const W = 440, H = Math.max(...layout.nodes.map(n => n.y)) + 60 || 80
  const R = 22
    ...
    <div className={styles.vizWrap}>
      ...
      <div className={styles.treeContainer} style={{ width: W, height: H }}>
        <svg style={{ position: 'absolute', top: 0, left: 0, width: W, height: H, pointerEvents: 'none' }}>
          {layout.edges.map(e => (
            <line key={e.id} x1={e.x1} y1={e.y1} x2={e.x2} y2={e.y2} stroke="rgba(255,245,228,0.50)" strokeWidth={1.5} />
          ))}
        </svg>
        {layout.nodes.map(n => {
...
function HashTableViz() {
  ...
  function animate(key: string, val: string, currentTable: HTable) {
    ...
    setActiveKey(key); setActiveVal(val); setRawHash(h); setBucket(b)
    setPhase('key');   setMsg(`Key: "${key}"`)

    setTimeout(() => { setPhase('hash'); setMsg(`hash("${key}") = ${h}  (sum of char codes)`) }, 700)
    setTimeout(() => { setPhase('mod');  setMsg(`${h} % ${HT_BUCKETS} = ${b}  →  bucket ${b}`) }, 1500)
    setTimeout(() => {
      setPhase('store')
      const collision = currentTable[b].length > 0 && !currentTable[b].some(e => e.key === key)
      setMsg(collision ? `Bucket ${b} occupied - append to chain. O(1).` : `Store "${key}" → "${val}" at bucket ${b}.`)
      setTable(t => {
        const n = t.map(c => [...c])
    ...
    setTimeout(() => {
      setPhase('idle'); setActiveKey(''); setActiveVal(''); setRawHash(null); setBucket(null)
      setMsg(IDLE_MSG)
    }, 3600)
  }
  ...
  function runLookup() {
    ...
    const h = htHash(key); const b = h % HT_BUCKETS

    setActiveKey(key); setRawHash(h); setBucket(b)
    setPhase('key');  setMsg(`lookup: "${key}"`)
    setTimeout(() => { setPhase('hash'); setMsg(`hash("${key}") = ${h}`) }, 700)
    setTimeout(() => { setPhase('mod');  setMsg(`${h} % ${HT_BUCKETS} = ${b}  →  check bucket ${b}`) }, 1500)
    setTimeout(() => { setPhase('store'); setLitKey(key); setMsg(`Found "${key}" in bucket ${b}. O(1).`) }, 2300)
    setTimeout(() => {
      setPhase('idle'); setActiveKey(''); setRawHash(null); setBucket(null); setLitKey(null)
      setMsg(IDLE_MSG)
    }, 3600)
  }
  ...
  function runDelete() {
    ...
    setActiveKey(key); setRawHash(h); setBucket(b)
    setPhase('key');  setMsg(`Delete: "${key}"`)
    setTimeout(() => { setPhase('hash'); setMsg(`hash("${key}") = ${h}`) }, 700)
    setTimeout(() => { setPhase('mod');  setMsg(`${h} % ${HT_BUCKETS} = ${b}  →  bucket ${b}`) }, 1500)
    setTimeout(() => {
    ...
    setTimeout(() => {
      setPhase('idle'); setActiveKey(''); setRawHash(null); setBucket(null); setLitKey(null)
      setMsg('A hash table maps keys to values using a hash function to find the right bucket in O(1).')
    }, 3600)
  }
  ...
  return (
    <div className={styles.vizWrap}>
      <div className={styles.htPipeline}>
        <div className={`${styles.htStep} ${phase === 'key' || phase === 'hash' || phase === 'mod' || phase === 'store' ? styles.htStepLit : ''}`}>
      ...
      <p className={styles.hint}>hash(key) = Σ char codes - index = hash % {HT_BUCKETS} - chaining for collisions</p>
      <div className={styles.htInputRow}>
        <input
          className={styles.htInput}
          placeholder="key"
...
const DEFINITIONS: Record<string, { desc: string; ops: string[]; when: string }> = {
  array: {
