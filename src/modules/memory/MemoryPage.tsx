...
function CallStackViz() {
  ...
  useEffect(() => { setIdx(0); setPlaying(false) }, [algo, n])
  ...
  const stepBack    = () => { setPlaying(false); setIdx(i => Math.max(0, i - 1)) }
  const stepForward = () => { setPlaying(false); setIdx(i => Math.min(frames.length - 1, i + 1)) }
  const togglePlay  = () => { if (playing) { setPlaying(false) } else if (!atEnd) setPlaying(true) }

  const range  = N_RANGE[algo]
  const topFrame = cf.stack.length > 0 ? cf.stack[cf.stack.length - 1] : null

  const inputLabel = algo === 'power'
    ? `2^${n}`
    : algo === 'sumofdigits'
    ? `n = ${SOD_PRESETS[n]}`
    : `n = ${n}`
    ...
    <div className={styles.csLayout}>
      ...
      <div className={styles.csLeft}>
        ...
        <div className={styles.csSection}>
          ...
          <div className={styles.sliderRow}>
            <input
              ...
              min={range.min} max={range.max} value={n}
        ...
        <div className={styles.csSection}>
          ...
          <PlaybackControls
            isPlaying={playing} atStart={idx === 0} atEnd={atEnd}
            onBack={stepBack} onPlay={togglePlay} onForward={stepForward}
          />
          <div className={styles.playbackMeta}>
            <span className={styles.stepCounter}>Step {idx + 1} / {frames.length}</span>
            <div className={styles.speedRow}>
              <span className={styles.speedLabel}>speed</span>
              <input
                type="range" className={styles.slider}
                min={0} max={4} value={speed}
          ...
          {atEnd && (
            <Button variant="pill" size="sm" onClick={() => { setIdx(0); setPlaying(false) }}>
              replay
            </Button>
          )}
        </div>
...
const HEAP_STEPS: HeapStep[] = [
  ...
  { stackVars: [{ name: 'name', ref: null, val: '"Alice"' }, { name: 'obj', ref: 'obj1', val: null }], heapObjects: [{ id: 'obj1', label: 'Object', fields: [['name', '"Alice"'], ['age', '25']] }], highlight: ['obj', 'obj1'], sym: 'let obj = {…}', detail: 'Objects live on the heap. obj on the stack holds a reference (memory address) pointing to the heap object.' },
  { stackVars: [{ name: 'name', ref: null, val: '"Alice"' }, { name: 'obj', ref: 'obj1', val: null }, { name: 'arr', ref: 'arr1', val: null }], heapObjects: [{ id: 'obj1', label: 'Object', fields: [['name', '"Alice"'], ['age', '25']] }, { id: 'arr1', label: 'Array', fields: [['0', '1'], ['1', '2'], ['2', '3']] }], highlight: ['arr', 'arr1'], sym: 'let arr = […]', detail: 'Arrays also live on the heap. arr holds a reference to the array object, not the array itself.' },
  { stackVars: [{ name: 'name', ref: null, val: '"Alice"' }, { name: 'obj', ref: 'obj1', val: null }, { name: 'arr', ref: 'arr1', val: null }, { name: 'copy', ref: 'obj1', val: null }], heapObjects: [{ id: 'obj1', label: 'Object', fields: [['name', '"Alice"'], ['age', '25']] }, { id: 'arr1', label: 'Array', fields: [['0', '1'], ['1', '2'], ['2', '3']] }], highlight: ['obj', 'copy', 'obj1'], sym: 'copy = obj', detail: 'copy = obj copies the reference, not the object. Both obj and copy point to the same heap address - mutating one affects the other.' },
]
...
function useStepThrough(length: number) {
  ...
  useEffect(() => { setIdx(0); setPlaying(false) }, [length])
  ...
  return {
    idx, atEnd,
    stepBack:    () => { setPlaying(false); setIdx(i => Math.max(0, i - 1)) },
    stepForward: () => { setPlaying(false); setIdx(i => Math.min(length - 1, i + 1)) },
    togglePlay:  () => { if (playing) setPlaying(false); else if (!atEnd) setPlaying(true) },
    replay:      () => { setIdx(0); setPlaying(false) },
    playing,
  }
}

function HeapViz() {
  const { idx, atEnd, stepBack, stepForward, togglePlay, replay, playing } = useStepThrough(HEAP_STEPS.length)
  ...
  return (
    <div className={styles.csLayout}>
      <div className={styles.csLeft}>
        ...
        <div className={styles.csSection}>
          <span className={styles.csSectionLabel}>playback</span>
          <PlaybackControls isPlaying={playing} atStart={idx === 0} atEnd={atEnd}
            onBack={stepBack} onPlay={togglePlay} onForward={stepForward} />
          <div className={styles.playbackMeta}>
            <span className={styles.stepCounter}>Step {idx + 1} / {HEAP_STEPS.length}</span>
          </div>
          {atEnd && <Button variant="pill" size="sm" onClick={replay}>replay</Button>}
        </div>
        <div className={styles.csSection}>
...
const SCOPE_STEPS: ScopeStep[] = [
  {
    ...
    sym: 'program starts', detail: 'The global scope is created. It exists for the entire lifetime of the program.',
  },
  {
    scopes: [{ name: 'global scope', vars: [['PI', '3.14'], ['greeting', '"hello"']], level: 0, active: true }],
    sym: 'globals declared', detail: 'Global variables are declared. They live in the global scope and are accessible from anywhere.',
  },
  {
    scopes: [
      { name: 'global scope', vars: [['PI', '3.14'], ['greeting', '"hello"']], level: 0, active: false },
      { name: 'outer()', vars: [], level: 1, active: true },
    ],
    sym: 'outer() called', detail: 'Calling outer() pushes a new frame. It has its own isolated scope.',
  },
  {
    scopes: [
      { name: 'global scope', vars: [['PI', '3.14'], ['greeting', '"hello"']], level: 0, active: false },
      { name: 'outer()', vars: [['x', '10'], ['y', '20']], level: 1, active: true },
    ],
    sym: 'x = 10, y = 20', detail: "outer() declares x=10 and y=20. These are local to outer's frame.",
  },
  {
    scopes: [
      { name: 'global scope', vars: [['PI', '3.14'], ['greeting', '"hello"']], level: 0, active: false },
      { name: 'outer()', vars: [['x', '10'], ['y', '20']], level: 1, active: false },
      { name: 'inner()', vars: [], level: 2, active: true },
    ],
    sym: 'inner() called', detail: 'inner() is called inside outer(). A new frame is pushed on top. Both outer and global scopes remain on the stack.',
  },
  {
    scopes: [
      { name: 'global scope', vars: [['PI', '3.14'], ['greeting', '"hello"']], level: 0, active: false },
      { name: 'outer()', vars: [['x', '10'], ['y', '20']], level: 1, active: false },
      { name: 'inner()', vars: [['x', '99'], ['z', '5']], level: 2, active: true },
    ],
    sym: 'x = 99 (shadow)', detail: "inner() declares its own x=99. This shadows outer's x=10 - inside inner(), x resolves to 99. outer's x is untouched.",
  },
  {
    scopes: [
      { name: 'global scope', vars: [['PI', '3.14'], ['greeting', '"hello"']], level: 0, active: false },
    ...
    sym: 'inner() returns', detail: "inner() returns. Its frame is popped and all its local variables are gone. outer's x=10 is in scope again.",
  },
  {
    scopes: [
      { name: 'global scope', vars: [['PI', '3.14'], ['greeting', '"hello"']], level: 0, active: true },
    ],
    sym: 'outer() returns', detail: "outer() returns. Its frame is popped. Only the global scope remains. x and y no longer exist.",
  },
]

function FunctionFramesViz() {
  const { idx, atEnd, stepBack, stepForward, togglePlay, replay, playing } = useStepThrough(SCOPE_STEPS.length)
      ...
      <div className={styles.csLeft}>
        ...
        <div className={styles.csSection}>
          ...
          <PlaybackControls isPlaying={playing} atStart={idx === 0} atEnd={atEnd}
            onBack={stepBack} onPlay={togglePlay} onForward={stepForward} />
          <div className={styles.playbackMeta}>
            <span className={styles.stepCounter}>Step {idx + 1} / {SCOPE_STEPS.length}</span>
          </div>
          {atEnd && <Button variant="pill" size="sm" onClick={replay}>replay</Button>}
        </div>
        <div className={styles.csSection}>
...
const REF_STEPS: RefStep[] = [
  { pVars: [], rVars: [], heapAge: null, litHeap: false, activeSide: null,
    sym: '', detail: 'Primitives (numbers, strings, booleans) are stored by value. Objects are stored by reference. Step through to see what that means.' },
  { pVars: [{ name: 'a', val: 5, lit: true }], rVars: [], heapAge: null, litHeap: false, activeSide: 'prim',
    sym: 'let a = 5', detail: 'a is a number. Its value (5) is stored directly in the variable.' },
  { pVars: [{ name: 'a', val: 5, lit: false }, { name: 'b', val: 5, lit: true }], rVars: [], heapAge: null, litHeap: false, activeSide: 'prim',
    sym: 'let b = a', detail: 'b = a copies the value. b gets its own independent copy of 5. a and b are separate.' },
  { pVars: [{ name: 'a', val: 5, lit: false }, { name: 'b', val: 10, lit: true }], rVars: [], heapAge: null, litHeap: false, activeSide: 'prim',
    sym: 'b = 10', detail: 'b is changed to 10. a is still 5. They are completely independent - changing b never affects a.' },
  { pVars: [{ name: 'a', val: 5, lit: false }, { name: 'b', val: 10, lit: false }], rVars: [{ name: 'p', lit: true }], heapAge: 25, litHeap: true, activeSide: 'ref',
    sym: 'let p = { age: 25 }', detail: 'An object is created on the heap. p holds a reference - a pointer to the object, not the object itself.' },
  { pVars: [{ name: 'a', val: 5, lit: false }, { name: 'b', val: 10, lit: false }], rVars: [{ name: 'p', lit: false }, { name: 'q', lit: true }], heapAge: 25, litHeap: false, activeSide: 'ref',
    sym: 'let q = p', detail: 'q = p copies the reference, not the object. Both p and q now point to the same heap object.' },
  { pVars: [{ name: 'a', val: 5, lit: false }, { name: 'b', val: 10, lit: false }], rVars: [{ name: 'p', lit: true }, { name: 'q', lit: true }], heapAge: 30, litHeap: true, activeSide: 'ref',
    sym: 'q.age = 30', detail: 'Mutating through q changes the shared heap object. p.age is now also 30 - both variables see the same change.' },
]

function ReferencesViz() {
  const { idx, atEnd, stepBack, stepForward, togglePlay, replay, playing } = useStepThrough(REF_STEPS.length)
      ...
      <div className={styles.csLeft}>
        ...
        <div className={styles.csSection}>
          ...
          <PlaybackControls isPlaying={playing} atStart={idx === 0} atEnd={atEnd}
            onBack={stepBack} onPlay={togglePlay} onForward={stepForward} />
          <div className={styles.playbackMeta}>
            <span className={styles.stepCounter}>Step {idx + 1} / {REF_STEPS.length}</span>
          </div>
          {atEnd && <Button variant="pill" size="sm" onClick={replay}>replay</Button>}
        </div>
        <div className={styles.csSection}>
